...
its C<.id>. An entry that can not be found is treated as already unbanned.
...
=head2 _unban_cidr_entry

Internal helper. Looks up the address-list entry for the CIDR range and deletes
it by its C<.id>. An entry that can not be found is treated as already
